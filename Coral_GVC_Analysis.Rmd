...
*Code DOI hosted on Zenodo* <a href="https://zenodo.org/badge/latestdoi/315749565"><img src="https://zenodo.org/badge/315749565.svg" alt="DOI"></a>

<br/>

## Coral Polyp and Design Setup {.tabset}
...
Initial carbonate chemistry values for the seawater in the three carboys used for the experimental pH treatments conducted in this study

```{r table 1 carbonate data, message=FALSE, warning=FALSE}

carbonate <- read.csv("Tables/CarbonateSW_data_10Dec19.csv", header = TRUE)
carbonate$pHt <- factor(carbonate$pHt, levels = c("8.2pH", "7.9pH", "7.6pH"))
carbonate$coral <- factor(carbonate$coral, levels = c("MCAV1", "MCAV2", "DAXI"))


## Create table 1 of carbonate parameters
Table1 <- carbonate %>% 
  group_by(pHt) %>% 
  summarise("Seawater pH" = round(mean(pHw), 2),
            "TA" = round(mean(TAi), 0),
            "DIC" = round(mean(DICi), 0),
            "pCO2" = round(mean(pCO2i), 0),
            "HCO3" = round(mean(HCO3i), 0),
            "CO3" = round(mean(CO3i), 0),
            "arag" = round(mean(aragi), 12))

## Rmarkdown table 1
kable(Table1[-1], booktabs = TRUE) %>% 
  kable_styling(font_size = 12, full_width = FALSE)

```

The nominal target pH values used were 8.2 (control), 7.9 and 7.6. The R package *seacarb* was used to calculate the various carbonate species using measured pH-NBS and total alkalinity as the known parameters. Concentrations in $\mu$mol kg^-1^; *p*CO~2~ in $\mu$atm.


### Table S1

#### **Table S1**
Calcification experiment carbonate parameters. Representative changes in carbonate chemistry during the 1.5 h calcification incubations reported in Figure 8. The nominal target pH values used were 8.2 (Control), 7.9 and 7.6. The R package *seacarb* was used to calculate the various carbonate species using measured pH-NBS and total alkalinity as the known parameters. Concentrations in µmol kg^-1^; *p*CO~2~ in µatm. MCAV 1 = *Montastraea cavernosa* No 1; MCAV 2 = *M. cavernosa* colony No 2; DAXI = *Duncanopsammia axifuga*.

```{r table S1 carbonate params, message=FALSE, warning=FALSE}

## Create table S1 of initial and final carbonate parameters
TableS1 <- carbonate %>% 
  group_by(coral, pHt) %>% 
  summarise("TAi" = round(mean(TAi), 0),
            "TAf" = round(mean(TAf), 0),
            "DICi" = round(mean(DICi), 0),
            "DICf" = round(mean(DICf), 0),
            "pCO2i" = round(mean(pCO2i), 0),
            "pCO2f" = round(mean(pCO2f), 0),
            "HCO3i" = round(mean(HCO3i), 0),
            "HCO3f" = round(mean(HCO3f), 0),
            "CO3i" = round(mean(CO3i), 0),
            "CO3f" = round(mean(CO3f), 0),
            "aragi" = round(mean(aragi), 1),
            "aragf" = round(mean(aragf), 1))

## Rmarkdown table S1
kable(TableS1[-1], booktabs = TRUE) %>% 
  kable_styling(font_size = 12, full_width = FALSE) %>% 
  pack_rows("MCAV 1", 1, 3) %>%
  pack_rows("MCAV 2", 4, 6) %>%
  pack_rows("DAXI", 7, 9)
...
step_all <- read.csv("Data/LD_step_pH_all_21Jun17.csv", header = TRUE)
step_all$water <- factor(step_all$water)
step_all$water <- factor(step_all$water, levels = c("8.2","7.9","7.6", "T"))
step_all$light <- factor(step_all$light, levels = c("L","D"))

## Subset for each species
cav_step <- subset(step_all, species == "MCAV") # M. cavernosa
dun_step <- subset(step_all, species == "DAXI") # D. axifuga

```

```{r stepwise pH plots, fig.align = 'center', fig.width = 8, fig.height = 4.2}

## MCAV plots
cav_step_plot <- ggplot(cav_step, aes(x = time, y = gvc, colour = water, group = water2)) +
  geom_point(size = 0.5) +
  xlab("") +
  ylab("") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), strip.background = element_blank(), strip.text = element_text(face = "italic"), text=element_text(family="Times New Roman")) +
  theme(legend.position = c(0.11, 0.67), legend.key.size = unit(0.2, "cm"), legend.text = element_text(size = 8)) +
  guides(colour = guide_legend(override.aes = list(linetype = 0, size = 3))) +
  xlim(0, 220) +
  ylim(6.9, 8.9) +
  facet_wrap(~ light, ncol = 1, labeller = as_labeller(c(L = "Light", D = "Dark"))) +
  scale_color_manual("", labels = c("8.2 pH","7.9 pH","7.6 pH"), values = c("#0072B2","#009E73","#D55E00","grey"), breaks = c("8.2","7.9","7.6")) +
  geom_line(aes(x = time, y = water2))


## DAXI plots
dun_step_plot <- ggplot(dun_step, aes(x = time, y = gvc, colour = water, group = water2)) +
  geom_point(size = 0.5) +
  xlab("") +
  ylab("") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), strip.background = element_blank(), strip.text = element_text(face = "italic"), text=element_text(family="Times New Roman")) +
  ...
  ylim(6.9, 8.9) +
  facet_wrap(~ light, ncol = 1, labeller = as_labeller(c(L = "Light", D = "Dark"))) +
  scale_color_manual("", labels = c("8.2 pH","7.9 pH","7.6 pH","T"), values = c("#0072B2","#009E73","#D55E00","grey")) +
  geom_line(aes(x = time, y = water2))


## Combine plots and save as final
plot_grid(cav_step_plot, dun_step_plot, labels = c("(a) M. cavernosa","(b) D. axifuga"), label_size = 12, scale = 0.95) +
  ggsave(file = "Figures/Fig5_stepwise_pH.pdf", height = 4.2, width = 8, useDingbats = FALSE)
...
```{r LD dataframes setup}

## Read in all L/D data for analyses
LD_stats <- read.csv("Data/LD_all_data_19Sep17.csv", header = TRUE)
LD_stats$treat <- factor(LD_stats$treat, levels = c("8.2", "7.9", "7.6")) # relevel pH treatments

## Subset dataframe
# light or dark
light_all <- subset(LD_stats, light.dark=="light")
dark_all <- subset(LD_stats, light.dark=="dark")

# MCAV light and DAXI light
light_cav <- subset(light_all, species=="Mcav")
light_dun <- subset(light_all, coral=="dun")

# MCAV dark and DAXI dark
dark_cav <- subset(dark_all, species=="Mcav")
dark_dun <- subset(dark_all, coral=="dun")

```


```{r MCAV summary model and labels}

## Linear model
# light values
cav_L_fig6_lm <- lm(pH ~ water, data = light_cav)
cav_l_slope <- cav_L_fig6_lm[["coefficients"]][["water"]]
cav_l_p <- summary(cav_L_fig6_lm)[["coefficients"]][2, 4]
cav_l_r <- summary(cav_L_fig6_lm)[["adj.r.squared"]]

# dark values
cav_D_fig6_lm <- lm(pH ~ water, data = dark_cav)
cav_d_slope <- cav_D_fig6_lm[["coefficients"]][["water"]]
cav_d_p <- summary(cav_D_fig6_lm)[["coefficients"]][2, 4]
cav_d_r <- summary(cav_D_fig6_lm)[["adj.r.squared"]]


## Labels for plot
cav_step_light_labs <- substitute(paste(Slope[light]==s, "; P"==p, "; R\U00B2"==r), list(s = format(cav_l_slope[1], digits = 2), p = format(cav_l_p[1], digits = 2), r = format(cav_l_r[1], digits = 2)))
cav_step_dark_labs <- substitute(paste(Slope[dark]==s, "; P < 0.01; R\U00B2"==r), list(s = format(cav_d_slope[1], digits = 2), p = format(cav_d_p[1], digits = 2), r = format(cav_d_r[1], digits = 2)))

```

```{r MCAV summary LD figure}

###### This is temporary, need to move it when finished!
sum <- summarySE(measurevar = "pH", groupvars = c("treat", "coral", "exp", "light.dark"), data = LD_stats)

## MCAV figure
cav_LD_summary_sum <- subset(sum, coral !="dun")
cav_LD_summary_sum$test <- paste(cav_LD_summary_sum$coral, cav_LD_summary_sum$exp, sep = "_")
cav_LD_summary_sum$colour <- paste(cav_LD_summary_sum$treat, cav_LD_summary_sum$light.dark, sep = "_")
cav_LD_summary_sum$shape <- paste(cav_LD_summary_sum$exp, cav_LD_summary_sum$light.dark, sep = "_")
cav_LD_summary_sum$treat <- as.numeric(levels(cav_LD_summary_sum$treat))[cav_LD_summary_sum$treat]

cav_LD_summary_plot <- ggplot(data = cav_LD_summary_sum) +
  geom_smooth(aes(x = treat, y = pH, group = light.dark), method="lm", se = FALSE, colour = "darkgrey") +
  geom_errorbar(aes(x = treat, ymin = pH - sd, ymax = pH + sd, colour = colour, group = test), width=0.07, size=0.8, position = position_dodge(width=0.07)) +
  geom_point(aes(x = treat, y = pH, shape = shape, colour = colour, group = test, fill = exp), size=2.5, position = position_dodge(width=0.07)) +
  scale_color_manual(values=c("#D55E00", "#D55E00", "#009E73","#009E73", "#0072B2", "#0072B2")) +
  scale_shape_manual(values=c(16,21,17,24,16,21,17,24)) +
  scale_fill_manual(values = c("white","white")) +
  geom_segment(aes(x = 7.45, xend = 7.75, y = 7.6, yend = 7.6), linetype = "longdash", colour = "#D55E00") +
  geom_segment(aes(x = 8.05, xend = 8.35, y = 8.2, yend = 8.2), linetype = "longdash", colour = "#0072B2") +
  geom_segment(aes(x = 7.75, xend = 8.05, y = 7.9, yend = 7.9), linetype = "longdash", colour = "#009E73") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), strip.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA, size=1), legend.position = "none", text=element_text(family="Times New Roman")) +
  scale_x_continuous(breaks=c(7.6, 7.9, 8.2)) +
  annotate("text", x = 7.8, y = 9, label = deparse(cav_step_light_labs), parse = TRUE, size = 3) +
  annotate("text", x = 7.8, y = 6.5, label = deparse(cav_step_dark_labs), parse = TRUE, size = 3) +
  ylab("GVC pH") +
  ylim(6.5, 9.1) +
  xlab("Seawater pH")
...
```{r DAXI summary model and labels}

## Linear model
# light values
dun_L_fig6_lm <- lm(pH ~ water, data = light_dun)
dun_l_slope <- dun_L_fig6_lm[["coefficients"]][["water"]]
dun_l_p <- summary(dun_L_fig6_lm)[["coefficients"]][2, 4]
dun_l_r <- summary(dun_L_fig6_lm)[["adj.r.squared"]]

# dark values
dun_D_fig6_lm <- lm(pH ~ water, data = dark_dun)
dun_d_slope <- dun_D_fig6_lm[["coefficients"]][["water"]]
dun_d_p <- summary(dun_D_fig6_lm)[["coefficients"]][2, 4]
dun_d_r <- summary(dun_D_fig6_lm)[["adj.r.squared"]]


## Labels for plot
dun_step_light_labs <- substitute(paste(Slope[light]==s, "; P < 0.01; R\U00B2"==r), list(s = format(dun_l_slope[1], digits = 2), p = format(dun_l_p[1], digits = 2), r = format(dun_l_r[1], digits = 2)))
dun_step_dark_labs <- substitute(paste(Slope[dark]==s, "; P < 0.01; R\U00B2"==r), list(s = format(dun_d_slope[1], digits = 2), p = format(dun_d_p[1], digits = 2), r = format(dun_d_r[1], digits = 2)))

```

```{r DAXI summary LD figure}

## DAXI figure
dun_LD_summary_sum <- subset(sum, coral == "dun")
dun_LD_summary_sum$test <- paste(dun_LD_summary_sum$coral, dun_LD_summary_sum$exp, sep = "_")
dun_LD_summary_sum$colour <- paste(dun_LD_summary_sum$treat, dun_LD_summary_sum$light.dark, sep = "_")
dun_LD_summary_sum$shape <- paste(dun_LD_summary_sum$exp, dun_LD_summary_sum$light.dark, sep = "_")
dun_LD_summary_sum$treat <- as.numeric(levels(dun_LD_summary_sum$treat))[dun_LD_summary_sum$treat]

dun_LD_summary_plot <- ggplot(data = dun_LD_summary_sum) +
  geom_smooth(aes(x = treat, y = pH, group = light.dark), method="lm", se = FALSE, colour = "darkgrey") +
  geom_errorbar(aes(x = treat, ymin = pH - sd, ymax = pH + sd, colour = colour, group = test), width=0.07, size=0.8, position = position_dodge(width=0.07)) +
  geom_point(aes(x = treat, y = pH, shape = shape, colour = colour, group = test, fill = exp), size=2.5, position = position_dodge(width=0.07)) +
  scale_color_manual(values=c("#D55E00", "#D55E00", "#009E73","#009E73", "#0072B2", "#0072B2")) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), strip.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA, size=1), legend.position = "none", text=element_text(family="Times New Roman")) +
  scale_x_continuous(breaks=c(7.6, 7.9, 8.2)) +
  annotate("text", x = 7.8, y = 9, label = deparse(dun_step_light_labs), parse = TRUE, size = 3) +
  annotate("text", x = 7.8, y = 6.5, label = deparse(dun_step_dark_labs), parse = TRUE, size = 3) +
  ylab("GVC pH") +
  ylim(6.5, 9.1) +
  xlab("Seawater pH")
...
```{r summary LD final plot, fig.align = 'center', fig.width = 8, fig.height = 3.5}

plot_grid(cav_LD_summary_plot, dun_LD_summary_plot,  labels = c("(a) M. cavernosa", "(b) D. axifuga"), scale = 0.87) +
  ggsave(file = "Figures/Fig6_LD_pH_summary.pdf", height = 3.5, width = 8, useDingbats = FALSE)
...
Mean (± SE) maximum and minimum pH~GVC~ values measured for each coral species at the three experimental pH treatments,
combining the values from both the "light–dark" and "stepwise" experimental protocols (Figs. **4, 5, 6**)

```{r table 2 }

table1 <- read.csv("Tables/LD_table2_30April20.csv", header = TRUE) # read in the table with data

## R markdown table
kable(table1[-1], booktabs = TRUE) %>% 
  kable_styling(font_size = 12, full_width = FALSE) %>% 
  pack_rows("M. cavernosa", 1, 4) %>%
  pack_rows("D. axifuga", 5, 8)

```

Mean (± S.E.) maximum and minimum pH~GVC~ values measured for each coral species at the three experimental pH treatments, combining the values from both the ‘Light-dark’ and ‘Stepwise’ experimental protocols (see Figures **5, 6** and **7**). The number of incubations included in each mean is represented by “n”. “Δ pH Light” and “Δ pH Dark” are the differences between treatment pH~SW~ and pH~GVC~ at that treatment. “Range of pH~GVC~” was calculated from the mean maximum and minimum for each pH experiment and represents the range of pH~GVC~ that a polyp could experience during a natural diel light cycle. “pH range” for Treatment pH is the range from 8.2 to 7.6, and for Maximum and Minimum pH~GVC~ in Light and Dark, it is the difference between pH~GVC~ at 8.2 and 7.6 seawater pH treatments.
...
#### **Table S2**
pH~GVC~ in the Light. Mean (±S.D.) pH~GVC~ measured during the light portion of the Light-Dark (LD) and Stepwise (STEP) experiments. Pairwise comparisons of pH~GVC~ between the two experiments used Tukey Honest Significant Differences. Asterisk denotes significant differences in mean pH~GVC~ between experiment types. n = the number of replicate runs of any given treatment protocol for each type of experiment.

```{r table S2 light GVC summary, message=FALSE, warning=FALSE}

## Create table S2 of initial and final carbonate parameters
TableS2 <- light_all %>% 
  group_by(species, treat, exp) %>% 
  summarise("Mean" = round(mean(pH), 2),
            "(+/-) SD Maximum" = round(sd(pH), 2),
            "n" = n(),
            "del pH" = round(mean(pH) - mean(water), 2))


## Rmarkdown table S2
kable(TableS2[-1], booktabs = TRUE) %>% 
  kable_styling(font_size = 12, full_width = FALSE) %>% 
  pack_rows("M. cavernosa", 1, 6) %>%
  pack_rows("D. axifuga", 7, 12)
...
#### **Table S3a**
Light pH~GVC~ ANOVA model. ANOVA model output assessing the effect of pH~SW~ on measured pH~GVC~ in the light during either Light-Dark or Stepwise experiments for all coral colonies.

***M. cavernosa* in Light-Dark trials**
```{r table S3a MCAV L/D light GVC anova}

## MCAV light ANOVAs
# Light data (L/D)
light_cava<-subset(light_cav, exp=="l/d")
lmcav_lm1<-aov(pH~treat, data=light_cava)
pander(summary(lmcav_lm1))

```

***M. cavernosa* in Stepwise trials**
```{r table S3a MCAV step light GVC anova}

# Light data (step)
light_cavb<-subset(light_cav, exp=="step")
lmcav_lm2<-aov(pH~treat, data=light_cavb)
pander(summary(lmcav_lm2))

```


***D. axifuga* in Light-Dark trials**
```{r table S3a DAXI L/D light GVC anova}

## DAXI light ANOVAs
# Light data (L/D)
light_duna<-subset(light_dun, exp=="l/d")
ldun_lm1<-aov(pH~treat, data=light_duna)
pander(summary(ldun_lm1))

```

***D. axifuga* in Stepwise trials**
```{r table S3a DAXI step light GVC anova}

# Light data (step)
light_dunb<-subset(light_dun, exp=="step")
ldun_lm2<-aov(pH~treat, data=light_dunb)
pander(summary(ldun_lm2))
...
#### **Table S3b**
Light pH~GVC~: Post Hoc Comparisons. Pairwise comparisons of mean pH~GVC~ among pH~SW~ treatments in the light for both types of experiments using Tukey Honest Significant Differences.  

```{r table S3b light GVC post hoc}

## MCAV light Tukey tests
# Light data (L/D)
lcav_ld_tukey <- as.data.frame(TukeyHSD(lmcav_lm1)[["treat"]])
lcav_ld_tukey$Experiment <- "Light-Dark"
lcav_ld_tukey$Coral <- "M. cavernosa"
lcav_ld_tukey$Treatments <- rownames(lcav_ld_tukey)
lcav_ld_tukey <- lcav_ld_tukey[c(6,5,7,4)]

# Light data (step)
lcav_step_tukey <- as.data.frame(TukeyHSD(lmcav_lm2)[["treat"]])
lcav_step_tukey$Experiment <- "Stepwise"
lcav_step_tukey$Coral <- "M. cavernosa"
lcav_step_tukey$Treatments <- rownames(lcav_step_tukey)
lcav_step_tukey <- lcav_step_tukey[c(6,5,7,4)]



## DAXI light Tukey tests
# Light data (L/D)
ldun_ld_tukey <- as.data.frame(TukeyHSD(ldun_lm1)[["treat"]])
ldun_ld_tukey$Experiment <- "Light-Dark"
ldun_ld_tukey$Coral <- "D axifuga"
ldun_ld_tukey$Treatments <- rownames(ldun_ld_tukey)
ldun_ld_tukey <- ldun_ld_tukey[c(6,5,7,4)]

# Light data (step)
ldun_step_tukey <- as.data.frame(TukeyHSD(ldun_lm2)[["treat"]])
ldun_step_tukey$Experiment <- "Stepwise"
ldun_step_tukey$Coral <- "D axifuga"
ldun_step_tukey$Treatments <- rownames(ldun_step_tukey)
ldun_step_tukey <- ldun_step_tukey[c(6,5,7,4)]

## Combine into 1 table
all_light_tukey <- rbind(lcav_ld_tukey, lcav_step_tukey, ldun_ld_tukey, ldun_step_tukey)


## Rmarkdown table 

kable(all_light_tukey[-1], booktabs = TRUE, row.names = FALSE) %>% 
  kable_styling(font_size = 12, full_width = FALSE) %>% 
  pack_rows("M. cavernosa", 1, 6) %>%
  pack_rows("D. axifuga", 7, 12)
...
#### **Table S4**
pH~GVC~ in the Dark. Mean (±S.D.) pH~GVC~ measured during the dark portion of the Light-Dark (LD) and Stepwise (STEP) experiments. Pairwise comparisons of pH~GVC~ between the two experiments used Tukey Honest Significant Differences. Asterisk denotes significant differences in mean pH~GVC~ between experiment types. n = the number of replicate runs of any given treatment protocol for each type of experiment.

```{r table S4 dark GVC summary, message=FALSE, warning=FALSE}

## Create table S4 of initial and final carbonate parameters
TableS4 <- dark_all %>% 
  group_by(species, treat, exp) %>% 
  summarise("Mean" = round(mean(pH), 2),
            "(+/-) SD Maximum" = round(sd(pH), 2),
...
#### **Table S5a**
Dark pH~GVC~ ANOVA model. ANOVA model output assessing the effect of pH~SW~ on measured pH~GVC~ in the dark during either Light-Dark or Stepwise experiments for all coral colonies.
  
***M. cavernosa* in Light-Dark trials**
```{r table S5a MCAV L/D dark GVC anova}

## MCAV dark ANOVAs
# Dark data (L/D)
dark_cava<-subset(dark_cav, exp=="l/d")
dmcav_lm1<-aov(pH~treat, data=dark_cava)
pander(summary(dmcav_lm1))

```

***M. cavernosa* in Stepwise trials**
```{r table S5a MCAV step dark GVC anova}

# Dark data (step)
dark_cavb<-subset(dark_cav, exp=="step")
dmcav_lm2<-aov(pH~treat, data=dark_cavb)
pander(summary(dmcav_lm2))

```

***D. axifuga* in Light-Dark trials**
```{r table S5a DAXI L/D dark GVC anova}

## DAXI dark ANOVAs
# Dark data (L/D)
dark_duna<-subset(dark_dun, exp=="l/d")
ddun_lm1<-aov(pH~treat, data=dark_duna)
pander(summary(ddun_lm1))

```

***D. axifuga* in Stepwise trials**
```{r table S5a DAXI step dark GVC anova}

# Dark data (step)
dark_dunb<-subset(dark_dun, exp=="step")
ddun_lm2<-aov(pH~treat, data=dark_dunb)
pander(summary(ddun_lm2))
...
#### **Table S5b**
Dark pH~GVC~: Post Hoc Comparisons. Pairwise comparisons of mean pH~GVC~ among pH~SW~ treatments in the dark for both types of experiments using Tukey Honest Significant Differences.  
  
```{r table S5b dark GVC post hoc}

## MCAV dark Tukey tests
# Dark data (L/D)
dcav_ld_tukey <- as.data.frame(TukeyHSD(dmcav_lm1)[["treat"]])
dcav_ld_tukey$Experiment <- "Light-Dark"
dcav_ld_tukey$Coral <- "M. cavernosa"
dcav_ld_tukey$Treatments <- rownames(dcav_ld_tukey)
dcav_ld_tukey <- dcav_ld_tukey[c(6,5,7,4)]

# Dark data (step)
dcav_step_tukey <- as.data.frame(TukeyHSD(dmcav_lm2)[["treat"]])
dcav_step_tukey$Experiment <- "Stepwise"
dcav_step_tukey$Coral <- "M. cavernosa"
dcav_step_tukey$Treatments <- rownames(dcav_step_tukey)
dcav_step_tukey <- dcav_step_tukey[c(6,5,7,4)]



## DAXI dark Tukey tests
# Dark data (L/D)
ddun_ld_tukey <- as.data.frame(TukeyHSD(ddun_lm1)[["treat"]])
ddun_ld_tukey$Experiment <- "Light-Dark"
ddun_ld_tukey$Coral <- "D axifuga"
ddun_ld_tukey$Treatments <- rownames(ddun_ld_tukey)
ddun_ld_tukey <- ddun_ld_tukey[c(6,5,7,4)]

# Dark data (step)
ddun_step_tukey <- as.data.frame(TukeyHSD(ddun_lm2)[["treat"]])
ddun_step_tukey$Experiment <- "Stepwise"
ddun_step_tukey$Coral <- "D axifuga"
ddun_step_tukey$Treatments <- rownames(ddun_step_tukey)
ddun_step_tukey <- ddun_step_tukey[c(6,5,7,4)]


## Combine into 1 table
all_dark_tukey <- rbind(dcav_ld_tukey, dcav_step_tukey, ddun_ld_tukey, ddun_step_tukey)


## Rmarkdown table 
kable(all_dark_tukey[-1], booktabs = TRUE, row.names = FALSE) %>% 
  kable_styling(font_size = 12, full_width = FALSE) %>% 
  pack_rows("M. cavernosa", 1, 6) %>%
  pack_rows("D. axifuga", 7, 12)
...
#### **Table S6**
Comparison of the response of pHGVC to decreases in pH~SW~ between the two experimental protocols used, Light-Dark vs Stepwise: Post Hoc Comparisons. Tukey Honest Significant Differences post hoc adjusted p values from 2-way ANOVAs assessing the effects of experimental design (Light-Dark vs Stepwise) and pH~SW~ treatment on either *M. cavernosa* or *D. axifuga* pH~GVC~ in the light and the dark. The pairwise comparisons between experiments at each treatment pH are only shown for the interaction term (Treatment * Experiment) because these were used in determining differences between experimental design on pH~GVC~.

```{r table S6 post hoc between experimental designs}

## MCAV experimental comparisons
# ANOVA 
lcav_lm <- aov(pH ~ treat * exp, data = light_cav) # Light
dcav_lm <- aov(pH ~ treat * exp, data = dark_cav) # Dark

# Light Tukey tests
cav_l_trt_tukey <- as.data.frame(TukeyHSD(lcav_lm)[["treat"]]) # extract treatment pairwise comparisons
cav_l_exp_tukey <- as.data.frame(TukeyHSD(lcav_lm)[["exp"]]) # extract experiment pairwise comparisons
cav_l_both_tukey <- as.data.frame(TukeyHSD(lcav_lm)[["treat:exp"]])[c(3,8,12),] # extract combined comparisons of interest only
cav_l_all_tukey <- rbind(cav_l_trt_tukey, cav_l_exp_tukey, cav_l_both_tukey) # combined all comparisons into single df
cav_l_all_tukey$Coral <- "M. cavernosa" # add species ID column
cav_l_all_tukey$Light <- "Light P adj." # add light column
cav_l_all_tukey$`Pairwise Comparisons` <- rownames(cav_l_all_tukey) # add column of row names to df
cav_l_all_tukey <- cav_l_all_tukey[c(5,6,7,4)] # subset for desired columns

# Dark Tukey tests
cav_d_trt_tukey <- as.data.frame(TukeyHSD(dcav_lm)[["treat"]]) # extract treatment pairwise comparisons
cav_d_exp_tukey <- as.data.frame(TukeyHSD(dcav_lm)[["exp"]]) # extract experiment pairwise comparisons
cav_d_both_tukey <- as.data.frame(TukeyHSD(dcav_lm)[["treat:exp"]])[c(3,8,12),] # extract combined comparisons of interest only
cav_d_all_tukey <- rbind(cav_d_trt_tukey, cav_d_exp_tukey, cav_d_both_tukey) # combined all comparisons into single df
cav_d_all_tukey$Coral <- "M. cavernosa" # add species ID column
cav_d_all_tukey$Light <- "Dark P adj." # add light column
cav_d_all_tukey$`Pairwise Comparisons` <- rownames(cav_d_all_tukey) # add column of row names to df
cav_d_all_tukey <- cav_d_all_tukey[c(5,6,7,4)] # subset for desired columns

# Combined tests 
cav_all_tukey <- rbind(cav_l_all_tukey, cav_d_all_tukey)
cav_all_tukey <- spread(cav_all_tukey, Light, `p adj`)



## DAXI experimental comparisons
# ANOVA 
ldun_lm <- aov(pH ~ treat * exp, data = light_dun) # Light
ddun_lm <- aov(pH ~ treat * exp, data = dark_dun) # Dark

# Light Tukey tests
dun_l_trt_tukey <- as.data.frame(TukeyHSD(ldun_lm)[["treat"]]) # extract treatment pairwise comparisons
dun_l_exp_tukey <- as.data.frame(TukeyHSD(ldun_lm)[["exp"]]) # extract experiment pairwise comparisons
dun_l_both_tukey <- as.data.frame(TukeyHSD(ldun_lm)[["treat:exp"]])[c(3,8,12),] # extract combined comparisons of interest only
dun_l_all_tukey <- rbind(dun_l_trt_tukey, dun_l_exp_tukey, dun_l_both_tukey) # combined all comparisons into single df
dun_l_all_tukey$Coral <- "D. axifuga" # add species ID column
dun_l_all_tukey$Light <- "Light P adj." # add light column
dun_l_all_tukey$`Pairwise Comparisons` <- rownames(dun_l_all_tukey) # add column of row names to df
dun_l_all_tukey <- dun_l_all_tukey[c(5,6,7,4)] # subset for desired columns

# Dark Tukey tests
dun_d_trt_tukey <- as.data.frame(TukeyHSD(ddun_lm)[["treat"]]) # extract treatment pairwise comparisons
dun_d_exp_tukey <- as.data.frame(TukeyHSD(ddun_lm)[["exp"]]) # extract experiment pairwise comparisons
dun_d_both_tukey <- as.data.frame(TukeyHSD(ddun_lm)[["treat:exp"]])[c(3,8,12),] # extract combined comparisons of interest only
dun_d_all_tukey <- rbind(dun_d_trt_tukey, dun_d_exp_tukey, dun_d_both_tukey) # combined all comparisons into single df
dun_d_all_tukey$Coral <- "D. axifuga" # add species ID column
dun_d_all_tukey$Light <- "Dark P adj." # add dark column
dun_d_all_tukey$`Pairwise Comparisons` <- rownames(dun_d_all_tukey) # add column of row names to df
dun_d_all_tukey <- dun_d_all_tukey[c(5,6,7,4)] # subset for desired columns

# Combined tests 
dun_all_tukey <- rbind(dun_l_all_tukey, dun_d_all_tukey)
dun_all_tukey <- spread(dun_all_tukey, Light, `p adj`)



## Combine into single table
all_tukey <- rbind(cav_all_tukey, dun_all_tukey)
all_tukey <- all_tukey[c(1,2,4,3)]


## Rmarkdown table 
kable(all_tukey, booktabs = TRUE, row.names = FALSE) %>% 
  kable_styling(font_size = 12, full_width = FALSE) %>% 
  pack_rows("M. cavernosa", 1, 7) %>%
  pack_rows("D. axifuga", 8, 14)
...
```{r DAXI pH rate v SW pH, fig.align='center', fig.height=3.5, fig.width=4, message=FALSE, warning=FALSE}
...
```{r DAXI pH rate v GVC pH, fig.align = 'center', fig.width = 4, fig.height = 3.5, message=FALSE, warning=FALSE}
...
```{r DAXI pH GVC pH v SW pH, fig.align = 'center', fig.width = 4, fig.height = 3.5, message=FALSE, warning=FALSE}
...
#### **Table S7**
Calcification experiment summary. Mean value (±S.D.) and sample size (n) of calcification rate, GVC O~2~, or pH~GVC~ measured through the calcification experiments for both species of coral. The calcification rates at pH 8.2 are the means of all measurements at that pH~SW~ including the 1 hour and 15 hour incubations from the all-Control runs. Significant differences are indicated by an asterisk (*) for differences between the control pH~SW~ and low pH~SW~ treatments by letters (a, b) for differences between time of exposure (1h, 15h). 

```{r table s7 of calcification mean values}

## Pull mean +/- SD rate and GVC values during calcification experiments
O2_calc_table <- O2_calc_sum[c(1:4)]
O2_calc_table$Model <- "Calcification rate"
O2_calc_table$Mean <- O2_calc_table$rate
O2_calc_table <- O2_calc_table[c(5,1,2,6,4)]

O2_pHVgvc_table <- O2_pHVgvc_sum[c(1:4)]
O2_pHVgvc_table$Model <- "GVC O2"
O2_pHVgvc_table$Mean <- O2_pHVgvc_table$gvc
O2_pHVgvc_table <- O2_pHVgvc_table[c(5,1,2,6,4)]

pH_calc_table <- pH_calc_sum[c(1,3:5)]
pH_calc_table$Model <- "Calcification rate"
pH_calc_table$Mean <- pH_calc_table$nmol
pH_calc_table <- pH_calc_table[c(5,1,2,6,4)]

pH_pHVgvc_table <- pH_pHVgvc_sum[c(1,3:5)]
pH_pHVgvc_table$Model <- "pHGVC"
pH_pHVgvc_table$Mean <- pH_pHVgvc_table$gvc
pH_pHVgvc_table <- pH_pHVgvc_table[c(5,1,2,6,4)]

dun_pH_calc_table <- dun_pH_calc_sum[c(1:2,4:6)]
dun_pH_calc_table$pH <- paste(dun_pH_calc_table$pH, dun_pH_calc_table$timeb, sep = " ")
dun_pH_calc_table$Model <- "Calcification rate"
dun_pH_calc_table$Mean <- dun_pH_calc_table$nmol
dun_pH_calc_table <- dun_pH_calc_table[c(6,1,3,7,5)]

dun_pH_pHVgvc_table <- dun_pH_pHVgvc_sum[c(1,3:6)]
dun_pH_pHVgvc_table$Model <- "pHGVC"
dun_pH_pHVgvc_table$Mean <- dun_pH_pHVgvc_table$gvc
dun_pH_pHVgvc_table <- dun_pH_pHVgvc_table[c(6,1,2,7,4)]


## Combine all data frames into single for table
calc_table_s7 <- rbind(O2_calc_table, O2_pHVgvc_table, pH_calc_table, pH_pHVgvc_table, dun_pH_calc_table, dun_pH_pHVgvc_table)
calc_table_s7$Mean <- round(calc_table_s7$Mean, 2)
calc_table_s7$sd <- round(calc_table_s7$sd, 2)


## Kable for Rmarkdown of mean calcification rate/GVC values
kable(calc_table_s7, booktabs = TRUE, row.names = FALSE) %>% 
  kable_styling(font_size = 12, full_width = FALSE) %>% 
  pack_rows("M. cavernosa (O2)", 1, 6) %>%
  pack_rows("M. cavernosa (pH)", 7, 12) %>%  
  pack_rows("D. axifuga (pH)", 13, 22)
